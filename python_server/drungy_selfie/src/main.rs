...
fn main()->Result<()> {
    ...
    let image = imread(&tmp_main_files[0], IMREAD_UNCHANGED).unwrap();
    ...
    if faces.len() > 0{
        for face in faces.iter(){
            ...
            let ad = format!("scale={}:{}", ((face.width as f32)*0.75).floor(), ((face.height as f32) * 0.75).floor());
            Command::new("ffmpeg")
                .arg("-i")
                .arg(hd.to_owned() + "/dface.png")
                .arg("-vf")
                .arg(format!("scale={}:{}",((face.width as f32) *1.25).floor(),((face.height as f32) *1.25).floor()))
            ...
            /*println!("not-darm: {:?}", */Command::new("ffmpeg")
                ...
                .arg(tmp_dface)
                .arg("-filter_complex")
                .arg(format!("[0:v][1:v] overlay={}:{}[o];[o][2:v]overlay={}:{}", face.x-face.width/4, face.y+face.height/4, face.x+((face.width as f32)/1.8).floor() as i32, face.y-((face.height as f32)/6.5).floor() as i32))
                .arg("-pix_fmt")
                .arg("yuv420p")
                .arg("-c:a")
                .arg("copy")
